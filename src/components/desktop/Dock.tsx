...
const SOCIAL_LINKS = [
  { id: 'github',   label: 'github',   icon: Github,    href: personalInfo.github },
  { id: 'linkedin', label: 'linkedin', icon: Linkedin,  href: personalInfo.linkedin },
  { id: 'email',    label: 'email',    icon: Mail,      href: `mailto:${personalInfo.email}` },
] as const;

const DOC_LINKS = [
  { id: 'resume',   label: 'resume',   icon: FileText,  href: withBasePath('/resume.pdf') },
] as const;
...
export default function Dock() {
  return (
    <div className="fixed bottom-0 left-0 right-0 h-16 z-50 flex items-center justify-center gap-6 bg-bg/80 backdrop-blur-sm border-t border-border">
      ...
      {DOC_LINKS.map(({ id, label, icon: Icon, href }) => (
        <div key={id} className="flex flex-col items-center gap-1">
          <motion.a
            href={href}
            target="_blank"
